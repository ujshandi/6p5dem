...
function boxclick3(box, category){
	if(box.checked) {
		if (category == 'DINAS PROVINSI') {
			create_kmzDinas(true);  		  
		}else if (category == 'DINAS KABUPATEN') {
			create_kmzDinas(false);
		}
...
function create_kmzDinas(prov){
	$.jsonp({
		...
		$.each(data, function(i, item){
				...
				if(prov){
					var kmlUrl = 'http://6p5dem.googlecode.com/svn/trunk/application/gis/kmz/province/'+color+'/'+kode+'.kmz';
				}else{
					var kmlUrl = 'http://6p5dem.googlecode.com/svn/trunk/application/gis/kmz/'+color+'/'+kode+'.kmz';
				}
				//alert(total/3 + ' ' + jumlah + ' ' + color);
				//window.alert("kode: "+kode+"jumlah: "+jumlah);
				//window.alert(i);
