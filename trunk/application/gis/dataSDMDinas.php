...
if($prov=='true'){
	...
	$stid = oci_parse($conn, ' select SUM(JUMLAH) as TOTAL from (Select a.KODEKABUP,count(*) as JUMLAH, b.NAMAKABUP from sdm_peg_dinas a inner join SDM_KABUPATEN b on b.KODEKABUP = a.KODEKABUP group by a.KODEKABUP, b.NAMAKABUP)'); //where ROWNUM <= 15 
	oci_execute($stid);
	...
	$stid = oci_parse($conn, ' select * from (Select a.KODEKABUP,count(*) as JUMLAH, b.NAMAKABUP from sdm_peg_dinas a inner join SDM_KABUPATEN b on b.KODEKABUP = a.KODEKABUP group by a.KODEKABUP, b.NAMAKABUP)');
	oci_execute($stid);
