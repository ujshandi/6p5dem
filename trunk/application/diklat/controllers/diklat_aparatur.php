...
class diklat_aparatur extends My_Controller {

	var $id = 'diklat_aparatur';
	...
	function __construct(){
		...
		$this->load->model('mdl_diklat_aparatur');
	}
	...
	public function index($sort_by ='KODE_DIKLAT', $sort_order ='ASC') ##sorting
	{
		...
		# get data
		$result = $this->mdl_diklat_aparatur->getData($data['numrow'], $offset, $data, $sort_by, $sort_order);
		
		# config pagination
		$config['base_url'] = base_url().'/'.$this->config->item('index_page').'/diklat_aparatur/index/'."$sort_by/$sort_order"; ##sorting
		...
		$this->load->view('diklat/diklat_aparatur_list', $data);
	...
	public function add(){
		$this->open();
		$this->load->view('diklat/diklat_aparatur_add');
		$this->close();
	}
	...
	public function edit($id){
		...
		$data['id'] = $id;
		$data['result'] = $this->mdl_diklat_aparatur->getDataEdit($id);
		$this->load->view('diklat/diklat_aparatur_edit', $data);
	...
	public function proses_edit(){
		...
		if ($this->form_validation->run() == FALSE){
			$data['result'] = $this->mdl_diklat_aparatur->getDataEdit($data['id']);
	...
	public function proses_delete($id){
		if($this->mdl_diklat_aparatur->delete($id)){
			redirect('diklat_aparatur');
		}else{
	...
	public function add_detail($kode_diklat){
		...
		if($this->mdl_diklat_aparatur->detail_diklat_exist($kode_diklat)){
			$data['action'] = 'update';
			$data['result'] = $this->mdl_diklat_aparatur->get_detail_diklat($kode_diklat);
		...
		$data['diklat'] = $this->mdl_diklat_aparatur->getDataEdit($kode_diklat);
		
		$this->load->view('diklat/diklat_aparatur_detail', $data);
	...
	public function proses_detail(){
		...
		$this->mdl_diklat_aparatur->insert_detail($data);
		redirect('diklat_aparatur');
