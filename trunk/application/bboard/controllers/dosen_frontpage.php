...
class Dosen_frontpage extends My_Frontpage {
	...
	function __construct(){
		...
		$this->load->model('mdl_dosen_front');
	...
	public function index()
	{
		...
		$data['jenis_dosen'] = $this->session->userdata($this->id.'jenis_dosen');
		$data['numrow'] = !empty($data['numrow'])?$data['numrow']:30;
		$offset = ($this->uri->segment(3))?$this->uri->segment(3):0;
		
		# get data
		$result = $this->mdl_dosen_front->getData($data['numrow'], $offset, $data);
		
		# config pagination
		$config['base_url'] = base_url().'/'.$this->config->item('index_page').'/dosen_frontpage/index/';
		...
		$this->load->view('info_dosen/dosen_list', $data);
	...
	public function search(){
		...
		$this->session->set_userdata($this->id.'jenis_dosen', $this->input->post('jenis_dosen'));
		
		redirect('dosen_frontpage');
	}
