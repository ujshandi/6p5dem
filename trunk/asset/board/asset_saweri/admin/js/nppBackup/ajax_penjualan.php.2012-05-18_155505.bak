<?php
	...
	function add_detail_1()
	{
		...
		if (isset($_POST['detail']))
		{
			...
			for($i=0; $i<$count_detail; $i++)
			{
				echo '
						<tr>
							<td>'.($i + 1).'</td>														<td>								'.$detail[$i]['kode_barang'].'								<input type="hidden" name="detail['.$i.'][kode_barang]" value="'.$detail[$i]['kode_barang'].'" />							</td>							
							<td>
		...
		foreach($data as $row){
			if(isset($data[$xx]['id_detail_pembelian'])){
				echo '
						<tr>
							<td>'.($i + 1).'</td>														<td>								'.$data[$xx]['kode_barang'].'								<input type="hidden" name="detail['.$i.'][kode_barang]" value="'.$data[$xx]['kode_barang'].'" />							</td>							
							<td>
								'.$data[$xx]['nama_barang'].'
	...
	function add_detail_2()
	{
		...
		$data['detail_kodebarang'] = $_POST['detail_kodebarang'];				$data['detail_idbarang'] = $_POST['detail_idbarang'];
		...
		if (isset($_POST['detail']))
		{
			...
			for($i=0; $i<$count_detail; $i++)
			{
				echo '
						<tr>
							<td>'.($i + 1).'</td>														<td>								'.$detail[$i]['kode_barang'].'								<input type="hidden" id="kode_barang'.$i.'" name="detail['.$i.'][kode_barang]" value="'.$detail[$i]['kode_barang'].'" />							</td>							
							<td>
								'.$detail[$i]['nama_barang'].'
				...
				echo '
						<tr>
							<td>'.($i + 1).'</td>														<td>								'.$data['detail_kodebarang'].'								<input type="hidden" id="kode_barang'.$i.'" name="detail['.$i.'][kode_barang]" value="'.$data['detail_kodebarang'].'" />															</td>							
							<td>
		...
		if (isset($_POST['detail']))
		{
			...
			for($x=0; $x<$count_detail; $x++)
			{
				if($x != $id)
				{
					echo '
							<tr>
								<td>'.($i + 1).'</td>																<td>									'.$detail[$x]['kode_barang'].'									<input type="hidden" name="detail['.$i.'][kode_barang]" value="'.$detail[$x]['kode_barang'].'" />																</td>								
								<td>
