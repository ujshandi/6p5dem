...
class elFinder {
	...
	public function __construct($opts) {
		...
		//echo "construct elfinder ok"; exit;
		// bind events listeners
		if (!empty($opts['bind']) && is_array($opts['bind'])) {
			foreach ($opts['bind'] as $cmd => $handler) {
				$this->bind($cmd, $handler);
			}
			//echo "not empty bind listener elfinder ok"; exit;
		}
        //echo "bind listener elfinder ok"; exit;
		// "mount" volumes
		if (isset($opts['roots']) && is_array($opts['roots'])) {
			...
			foreach ($opts['roots'] as $i => $o) {
				...
				if (class_exists($class)) {
					$volume = new $class();
                     //echo "bind class elfinder ok"; exit;
					 //var_dump($volume);exit;
					 //var_dump($o);exit;
					if ($volume->mount($o)) {
						// unique volume id (ends on "_") - used as prefix to files hash
						//echo "bind volume elfinder ok"; exit;
						$id = $volume->id();
						//var_dump($volume);exit;
