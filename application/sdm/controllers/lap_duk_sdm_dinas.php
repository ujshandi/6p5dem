<?php 
class Lap_duk_sdm_dinas extends My_Controller{
    ...
    function index(){
		$this->open();
		$config['base_url'] = base_url().'/'.$this->config->item('index_page').'/lap_duk_sdm_dinas/index/';
		$data['option_provin'] = $this->mdl_sdm_dinas->getprovin();
		$this->load->view('laporan/sdm_duk_laporan_dinas',$data);
		$this->close();
	}
	...
	function pdf(){
		...
		$pdfdata = $this->laporan->get_duk_sdm_dinas($prov, $kab);
		if (count($pdfdata)==0){
		...
		$posY = 11;
		$posX = 10;
		$this->our_pdf->text($posX,$posY,'Laporan DUK SDM Aparatur Dinas');
		...
		$this->our_pdf->Output("SDM_Dinas_duk.pdf","I");
