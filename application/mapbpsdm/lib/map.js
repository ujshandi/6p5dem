...
//bagian untuk shp awal
var shpLayer = new OpenLayers.Layer.WMS( "admKecBandung", "http://localhost:8090/geoserver/wms", {layers: 'admKecBandung', transparent: true, format:'image/png'}, {opacity: 0.8});
	map.events.register('zoomend', this, function() {
		if (map.getZoom() < 16) {
			if (document.getElementById("check_admKec").checked == true) {
				shpLayer.setVisibility(true);
			} else {
				shpLayer.setVisibility(false);
			}
...
//bagian untuk shp akhir


map.addLayers([osm, gmap, gphy, lyrTopo, shpLayer]);

//bagian shp lagi awal
	var showAdmKecLayer = false;
	
	function showAdmKec() {
		if (!showAdmKecLayer) {
			document.getElementById("check_admKec").checked = true;
			shpLayer.setVisibility(true);
			showAdmKecLayer = true;
		} else {
			document.getElementById("check_admKec").checked = false;
			shpLayer.setVisibility(false);
			showAdmKecLayer = false;
		}
	}
//bagian shp lagi akhir
