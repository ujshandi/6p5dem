...
class diklat_udara extends My_Controller {
	...
	function __construct(){
		...
		$this->load->model('mdl_diklat_udara');
	}
	...
	public function index($sort_by ='KODE_DIKLAT', $sort_order ='ASC') ##sorting
	{
		...
		# get data
		$result = $this->mdl_diklat_udara->getData($data['numrow'], $offset, $data, $sort_by, $sort_order);
		
		# config pagination
		$config['base_url'] = base_url().'/'.$this->config->item('index_page').'/diklat_udara/index/'."$sort_by/$sort_order"; ##sorting
		...
		$this->load->view('diklat/diklat_udara_list', $data);
	...
	public function add(){
		$this->open();
		$this->load->view('diklat/diklat_udara_add');
		$this->close();
	}
	...
	public function edit($id){
		...
		$data['id'] = $id;
		$data['result'] = $this->mdl_diklat_udara->getDataEdit($id);
	...
	public function proses_edit(){
		...
		if ($this->form_validation->run() == FALSE){
			$data['result'] = $this->mdl_diklat_udara->getDataEdit($data['id']);
			$this->load->view('diklat/diklat_udara_edit',$data);
		}else{
	...
	public function proses_delete($id){
		if($this->mdl_diklat_udara->delete($id)){
			redirect('diklat_udara');
		}else{
	...
	public function add_detail($kode_diklat){
		...
		if($this->mdl_diklat_udara->detail_diklat_exist($kode_diklat)){
			$data['action'] = 'update';
			$data['result'] = $this->mdl_diklat_udara->get_detail_diklat($kode_diklat);
		...
		$data['diklat'] = $this->mdl_diklat_udara->getDataEdit($kode_diklat);
		
		$this->load->view('diklat/diklat_udara_detail', $data);
	...
	public function proses_detail(){
		...
		$this->mdl_diklat_udara->insert_detail($data);
		redirect('diklat_udara');
