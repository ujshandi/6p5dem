...
class upt extends My_Controller {
	...
	public function index($sort_by ='NAMA_UPT',$sort_order ='ASC') ##sorting
	{
		...
		##sorting
		$result = $this->mdl_upt->getData($data['numrow'], $offset, $data, $sort_by, $sort_order);
		
		# config pagination
		$config['base_url'] = base_url().'/'.$this->config->item('index_page').'/upt/index/'."$sort_by/$sort_order"; ##sorting
