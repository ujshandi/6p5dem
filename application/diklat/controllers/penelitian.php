...
class penelitian extends My_Controller {
	...
	public function index($sort_by ='JUDUL_PENELITIAN', $sort_order ='ASC') ##sorting
	{
		...
		$data['fields'] = array (
			...
			'TGL_PUBLIKASI' => 'TANGGAL PUBLIKASI',
		);
		...
		##sorting
		$result = $this->mdl_penelitian->getData($data['numrow'], $offset, $data, $sort_by, $sort_order);
		
		# config pagination
		$config['base_url'] = base_url().'/'.$this->config->item('index_page').'/penelitian/index/'.'/alumni/index/'."$sort_by/$sort_order"; ##sorting
