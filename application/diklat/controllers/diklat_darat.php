...
class diklat_darat extends My_Controller {
	...
	public function index($sort_by ='KODE_DIKLAT', $sort_order ='ASC') ##sorting
	{
		...
		# get data
		$result = $this->mdl_diklat_darat->getData($data['numrow'], $offset, $data, $sort_by, $sort_order);
		
		# config pagination
		$config['base_url'] = base_url().'/'.$this->config->item('index_page').'/diklat_darat/index/'."$sort_by/$sort_order"; ##sorting
	...
	public function add_detail($kode_diklat){
		...
		if($this->mdl_diklat_darat->detail_diklat_exist($kode_diklat)){
			$data['action'] = 'update';
			$data['result'] = $this->mdl_diklat_darat->get_detail_diklat($kode_diklat);
		...
		$data['diklat'] = $this->mdl_diklat_darat->getDataEdit($kode_diklat);
		
		$this->load->view('diklat/diklat_darat_detail', $data);
	...
	public function proses_detail(){
		...
		$this->mdl_diklat_darat->insert_detail($data);
		redirect('diklat_darat');
